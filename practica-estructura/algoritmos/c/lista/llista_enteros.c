...
if ( posicion < 0 || posicion > lista->elementos ){
	...
	aux = cursor1->valor;
	ins(lista,elemento,posicion);
	eliminar(lista, posicion-1);
	return aux;
}
...
int valor2;
	...
	valor1 = buscar(lista, posicion1);
	valor2 = buscar(lista, posicion2);

	// Llamar a la funcion reemplazar 2 veces
	reemplazar(lista, valor1, posicion2);
	reemplazar(lista, valor2, posicion1);
}
